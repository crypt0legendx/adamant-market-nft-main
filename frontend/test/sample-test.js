...
describe("NFTMarket", function() {
  it("Should create and execute market sales", async function() {
    ...
    console.log("owner address:", owner.address, addr1.address, addr2.address);

    const Master = await ethers.getContractFactory("Master")
    const master = await Master.deploy("0x76Ac52c1a8709B65e4e60D0c54802b70f8bb46f8");
    await master.deployed()
    console.log("master address:", master.address)
    const Oracle = await ethers.getContractFactory("Oracle")
    const oracle = await Oracle.deploy(master.address)
